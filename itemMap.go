...
func NewItemMap(b ...byte) *ItemMap {
	if len(b) != 0 {
		return getItemMap(&b)
	}
	x, err := net()
	...
	return unmarshallRawToItemMap(r.Items)
}
...
// unmarshallRawToItemMap returns a Item map from the raw bytes contained within the Nintendo XHR HTTP response.
